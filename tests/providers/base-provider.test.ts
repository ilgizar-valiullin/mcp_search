...
describe('BaseProvider', () => {
  ...
  it('should skip doSearch for queued requests when provider becomes unhealthy', async () => {
    const error = new Error('API down');
    provider.doSearchFn.mockRejectedValue(error);
    ...
    // q1, q2, q3 all reach doSearch and fail with API down
    // (provider becomes unhealthy after 3rd consecutive error)
    for (let i = 0; i < 3; i++) {
      expect(results[i].status).toBe('rejected');
    }
    // q4 is queued behind q3, wakes up to find unhealthy, throws without doSearch
    expect(results[3].status).toBe('rejected');
    if (results[3].status === 'rejected') {
      expect((results[3] as PromiseRejectedResult).reason.message).toBe(
        'Provider Dummy is currently unhealthy',
      );
    }
    // Only 3 calls reached doSearch
    expect(provider.doSearchFn).toHaveBeenCalledTimes(3);
  });
